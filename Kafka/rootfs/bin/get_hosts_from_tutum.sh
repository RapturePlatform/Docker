parseServiceLinks() {
    ...
    do
        curl -s -H "Authorization: $DOCKERCLOUD_AUTH" -H "Accept: application/json" ${DOCKERCLOUD_REST_HOST}${service_link} > /tmp/cur_service
...
if [ -n "${DOCKERCLOUD_AUTH}" ] && [ -n "${DOCKERCLOUD_API_CALLS_FOR_DNS}" ]
then
    curl -s -H "Authorization: $DOCKERCLOUD_AUTH" -H "Accept: application/json" ${DOCKERCLOUD_REST_HOST}/api/v1/container/ > /tmp/containers.raw
